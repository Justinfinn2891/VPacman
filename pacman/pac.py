...
class Ghost:
    ...
    def check_collisions(self):
        ...
        self.turns = [False, False, False, False]  # Initialize turns: [left, right, up, down]

        
        # Define valid moves based on conditions
        if 0 < self.center_x // 30 < 29:
    # Movement conditions based on level layout and ghost status (dead or alive)
            if level[(self.center_y - num3) // num1][self.center_x // num2] < 3 or \
            (level[(self.center_y - num3) // num1][self.center_x // num2] == 9 and (self.dead or not self.in_box)):
                self.turns[2] = True  # Up
                
            if level[self.center_y // num1][(self.center_x - num3) // num2] < 3 or \
            (level[self.center_y // num1][(self.center_x - num3) // num2] == 9 and (self.dead or not self.in_box)):
            ...
            # Directional adjustments for exit when within box boundaries
            if self.direction in [2, 3] and 12 <= self.center_x % num2 <= 18:
                if level[(self.center_y + num3) // num1][self.center_x // num2] < 3:
                    self.turns[3] = True
                if level[(self.center_y - num3) // num1][self.center_x // num2] < 3:
                    self.turns[2] = True

            if self.direction in [0, 1] and 12 <= self.center_x % num2 <= 18:
            ...
            self.turns[0] = self.turns[1] = True  # Allow movement on the edges to avoid getting stuck

        # Update in_box status based only on coordinates, not movement restrictions
        if 350 < self.x_pos < 550 and 370 < self.y_pos < 480:
            self.in_box = self.dead  # Only stay in box if dead
        else:
            self.in_box = False  # Not in box once alive

# Return the possible turns and the current in_box status
    ...
    def move_pinky(self):
        ...
        if self.direction == 0:  # Moving right
            if self.target[0] > self.x_pos and self.turns[0]:  # Move right if target is to the right
                self.x_pos += self.speed
            elif self.target[1] > self.y_pos and self.turns[3]:  # Otherwise, move down toward target if allowed
                self.direction = 3
                self.y_pos += self.speed
            elif self.target[1] < self.y_pos and self.turns[2]:  # Or move up if allowed
                self.direction = 2
                self.y_pos -= self.speed
            elif self.turns[1]:  # Else, move left if blocked
                self.direction = 1
                self.x_pos -= self.speed
            else:  # Default to right movement if blocked and no other turn is allowed
                self.x_pos += self.speed

        elif self.direction == 1:  # Moving left
            if self.target[0] < self.x_pos and self.turns[1]:  # Move left toward target
                self.x_pos -= self.speed
            elif self.target[1] > self.y_pos and self.turns[3]:  # Move down if target is below
                self.direction = 3
                self.y_pos += self.speed
            elif self.target[1] < self.y_pos and self.turns[2]:  # Move up if target is above
                self.direction = 2
                self.y_pos -= self.speed
            elif self.turns[0]:  # Default to moving right if blocked
                self.direction = 0
                self.x_pos += self.speed

        elif self.direction == 2:  # Moving up
            if self.target[1] < self.y_pos and self.turns[2]:  # Move up toward target
                self.y_pos -= self.speed
            elif self.target[0] > self.x_pos and self.turns[0]:  # Move right if target is to the right
                self.direction = 0
                self.x_pos += self.speed
            elif self.target[0] < self.x_pos and self.turns[1]:  # Move left if target is to the left
                self.direction = 1
                self.x_pos -= self.speed
            elif self.turns[3]:  # Default to down if no other direction is allowed
                self.direction = 3
                self.y_pos += self.speed

        elif self.direction == 3:  # Moving down
            if self.target[1] > self.y_pos and self.turns[3]:  # Move down toward target
                self.y_pos += self.speed
            elif self.target[0] > self.x_pos and self.turns[0]:  # Move right if target is to the right
                self.direction = 0
                self.x_pos += self.speed
                ...
                self.direction = 1
                self.x_pos -= self.speed
            elif self.turns[2]:  # Default to up if no other direction is allowed
                self.direction = 2
                self.y_pos -= self.speed

        # Handle screen wrapping to keep Pinky within bounds
        if self.x_pos < -30:
            self.x_pos = 900
        elif self.x_pos > 900:
            self.x_pos = -30  # Fix typo in original code
...
def draw_misc():
    ...
    if game_won:
        pygame.draw.rect(screen, 'white', [50, 200, 800, 300], 0, 10)
        pygame.draw.rect(screen, 'dark gray', [70, 220, 760, 260], 0, 10)
        gameover_text = font.render('Victory! Space bar for next level!', True, 'green')
        screen.blit(gameover_text, (100, 300))  
...
while run:
    ...
    if powerup and player_circle.colliderect(blinky.rect) and eaten_ghost[0] and not blinky.dead:
       if(player_circle.colliderect(blinky.rect) and not blinky.dead) or (player_circle.colliderect(inky.rect) and not inky.dead) \
          or (player_circle.colliderect(pinky.rect) and not pinky.dead) or (player_circle.colliderect(clyde.rect) and not clyde.dead):
              if lives > 0:
                lives -= 1
                startup_counter = 0
                powerup = False
